...
function setConnected(connected) {
  ...
  document.querySelector("#setupCard").hidden = connected;
...
async function sendLocation(accuracy = 5, quiet = false) {
  ...
  try {
    ...
    const data = await api(`/api/clients/${client.id}/location`, { method: "POST", body: JSON.stringify(body) });
    client = data.client;
    sessionStorage.setItem("rtls-client", JSON.stringify(client));
    renderClient();
    ...
    if (error.status === 404) {
      clearClient();
      return toast("서버 연결이 초기화되었습니다. 다시 연결해 주세요.");
    }
    toast(error.message);
  }
}
...
function renderClient() {
  ...
  document.querySelector("#statusMessage").textContent = danger ? "안전한 장소로 즉시 이동하세요." : "위치 정보를 관제센터와 공유하고 있습니다.";
}
...
async function sendSos() {
  if (!client) return;
  try {
    await api(`/api/clients/${client.id}/sos`, { method: "POST", body: JSON.stringify({ message: "사용자 앱 긴급 구조 요청" }) });
...
async function disconnect() {
  if (!client) return;
  try { await api(`/api/clients/${client.id}/disconnect`, { method: "POST", body: "{}" }); } catch (_) {}
  clearClient();
}
...
async function restoreSession() {
  try {
    const stored = JSON.parse(sessionStorage.getItem("rtls-client"));
    if (!stored?.id) return;
    const state = await api("/api/state");
    const current = state.clients.find(item => item.id === stored.id);
    if (!current) return clearClient();
    client = current;
