...
| 2026-06-14 | 1.40 | 안전 분석·Webhook·재난문서 검색·UI 회귀검사 반영 | 김지훈 |
...
<div align="center">
  <sub>[Figure 2-1] 초기 핵심 흐름 Use Case Diagram. 아래 Revised Use Case Structure가 현재 구현 범위의 기준이다.</sub>
</div>
...
flowchart LR
    ...
    UC14((Danger Area and Device Alerts))
    UC15((Movement Analytics and Facilities))
    UC16((Group and Audit Management))
    UC17((Guardian Safety Check))
    UC18((External Alert Automation))
    UC19((Disaster Knowledge Search))
...
- Danger Area and Device Alert Management
- Movement Analytics and Safety Facility Lookup
- Group, Alert Filter and Audit Management
- Guardian Safety Check
- External Webhook Automation
- Disaster Knowledge Search
...
# Use case #10~#19 : Extended Operations
...
| Danger Area and Device Alert Management | Operator / System | 원형·다각형 구역과 진입·이탈·신호·배터리·무동작 경고 관리 |
| Movement Analytics and Safety Facilities | Operator | 이동·정차·방문 통계와 가까운 대피소·병원·소방서 조회 |
| Group and Audit Management | National / Regional Operator | 사용자 그룹, 알림 검색과 관리자 작업 감사 기록 |
| Guardian Safety Check | Client / Guardian | 사용자 동의 기반 보호자 연결과 제한된 최신 안전 상태 조회 |
| External Alert Automation | System / n8n | 중요 이벤트의 비동기 HMAC 서명 Webhook 전달 |
| Disaster Knowledge Search | Operator | 출처가 표시된 재난 행동요령 문단 검색 |
...
<div align="center">
  <sub>[Figure 3-1] 초기 핵심 모듈 Domain Diagram. 아래 Revised Domain Model과 엔티티 표가 현재 구현 기준이다.</sub>
</div>
...
```mermaid
classDiagram
    UserAccount "1" --> "0..1" Client : connects
    UserAccount "0..*" --> "0..*" UserAccount : guardianLinks
    Client "1" --> "0..*" RoutePoint : records
    Client "1" --> "0..*" SosEvent : creates
    Client "0..*" --> "0..*" DangerArea : enters
    Client "0..*" --> "0..*" ClientGroup : belongsTo
    MonitoringOperator "1" --> "0..*" SosEvent : handles
    Client "1" --> "0..*" SystemEvent : causes
    Region "1" --> "0..*" PublicAlert : contains
...
| PolygonDangerArea | `id`, `name`, `points`, `severity` | 여러 꼭짓점으로 구성된 비정형 위험 지역 정의 |
| ClientGroup | `id`, `name`, `color`, `memberIds` | 여러 사용자의 관제 분류와 지도 필터 |
| AuditLog | `actorId`, `action`, `target`, `regionId`, `createdAt` | 관리자 변경 작업 추적 |
...
| UserAccount | `id`, `username`, `passwordHash`, `role`, `regionId`, `birthDate`, `gender` | 사용자·보호자·관리자 계정, 역할과 회원 프로필 관리 |
| GuardianLink | `guardianId`, `protectedUserId`, `createdAt` | 사용자 동의에 따른 보호자와 보호 대상의 다대다 연결 |
...
- 한 명의 보호자는 여러 사용자를, 한 명의 사용자는 여러 보호자를 연결할 수 있다. (`N:M`)
...
| Guardian Links | SQLite Junction Table | 보호자 연결·해제와 보호 대상 조회 | 복합 기본키로 중복을 막고 계정 삭제 시 연쇄 정리 |
...
| Polygon Point Test | Ray Casting | 다각형 꼭짓점 수를 `p`라 할 때 포함 판정 `O(p)` |
| Client Groups | Hash Map + Member Set | 그룹 조회 평균 `O(1)`, 구성원 필터 `O(k)` |
| Movement Statistics | Bounded Route Scan | 최대 500개 위치를 한 번 순회해 거리·정차·방문 빈도 계산 `O(k)` |
| Webhook Delivery | Bounded Queue + Worker | 위치·SOS 요청과 외부 자동화 지연을 분리하고 큐 크기를 제한 |
| Disaster Retrieval | Inverted Document Frequency Scoring | 질문 토큰과 문서 문단의 관련도를 계산하고 출처와 함께 상위 결과 반환 |
...
- `ACKNOWLEDGED`: 관제자가 요청을 접수하고 담당자로 배정된 상태
- `DISPATCHED`: 담당자가 현장으로 출동 중인 상태
- `RESOLVED`: 구조 지원이 완료된 상태
- `CANCELLED`: 요청이 취소된 상태

현재 시스템은 이전 상태별 허용 전이를 서버에서 검증하여 `OPEN`에서 바로
`DISPATCHED` 또는 `RESOLVED`로 변경하는 요청과 완료된 SOS의 재처리를 거부한다.
상태 변경마다 담당자, 처리 메모와 변경 시각을 SOS 이력에 추가한다.
...
<div align="center">
  <sub>[Figure 4-1] 2025년 초기 UI 목표안. 현재 구현은 아래 검증 화면을 기준으로 한다.</sub>
</div>

### Current Implemented Interface

![Current Monitoring Dashboard](e2e/monitor.spec.js-snapshots/monitor-dashboard-desktop-chromium-darwin.png)

<div align="center">
  <sub>[Figure 4-2] 현재 데스크톱 전국 관제 화면. Leaflet 시·도 지도, 지역·그룹 필터, 활성 알림, 사용자 상세와 이벤트 영역을 포함한다.</sub>
</div>

![Current Mobile Monitoring Dashboard](e2e/monitor.spec.js-snapshots/monitor-mobile-mobile-chromium-darwin.png)

<div align="center">
  <sub>[Figure 4-3] 현재 모바일 반응형 관제 화면. 가로 넘침과 주요 한글 UI 표시를 Playwright로 검사한다.</sub>
</div>

초기 프로토타입의 도로 배경 지도, 별도 데스크톱 연결 창과 독립 위험 팝업은 현재
MVP에 그대로 구현된 화면이 아니다. 현재는 사용자 웹 앱과 전국 관제 대시보드로
통합했으며, 위험 상태는 사용자 앱 내부 경고와 관제 활성 알림으로 표시한다.
...
MFA, 영구 보존되는 보안 감사 로그, 백업과 법적 보존 기간 기반 자동 파기 정책은 구현하지 않았다.
...
| R-04 | 위험 경고 | UC-04 | SystemEvent | 사용자 위험 화면, SSE | 도메인 테스트 완료 |
| R-05 | SOS 요청 | UC-05 | SosEvent | SOS API | 완료 |
| R-06 | 중앙 관제 | UC-06 | Snapshot, Publish-Subscribe | state API, dashboard | Playwright 화면 검사 완료 |
...
| R-16 | 위험구역·기기 경고 | UC-14 | Polygon, Heartbeat, Battery | danger/device APIs와 이벤트 | 완료 |
| R-17 | 이동 분석·안전시설 | UC-15 | Route Scan, Distance Sort | analytics/facilities API | 완료 |
| R-18 | 그룹·감사 관리 | UC-16 | Group Map, Audit List | group API와 snapshot | 완료 |
| R-19 | 보호자 조회 | UC-17 | GuardianLink, Role Policy | guardian API | 완료 |
| R-20 | 외부 알림 자동화 | UC-18 | WebhookNotifier | 서명·선별 테스트 | 완료 |
| R-21 | 재난문서 검색 | UC-19 | IDF Retrieval | disaster knowledge API | 완료 |
