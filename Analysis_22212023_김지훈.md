...
| Item | Description |
|---|---|
| Summary | 사용자가 시스템에 접속하여 고유 ID를 부여받는다. |
| Scope | Real Time Location System |
| Level | User Level |
| Primary Actor | Client |
| Preconditions | 시스템 서버가 실행 중이어야 한다. |
| Trigger | 사용자가 이름을 입력하고 연결 버튼을 누른다. |
| Success Post Condition | 서버와 연결되고 사용자 ID가 생성된다. |
...
| Step | Action |
|---|---|
| 1 | 사용자가 웹 앱을 열고 이름을 입력한다. |
| 2 | 사용자가 연결 버튼을 누르면 클라이언트가 서버에 등록을 요청한다. |
| 3 | 서버가 클라이언트를 승인한다. |
| 4 | 서버가 사용자 ID를 생성한다. |
| 5 | 사용자가 시스템에 접속된다. |
...
| 3 | 오래된 위치 여부는 현재 구현에서 별도 판정하지 않으며 향후 `STALE` 상태를 추가한다. |
...
| 3 | 현재 상태에서 허용되지 않는 SOS 상태 전이는 운영 버전에서 거부해야 한다. |
...
| MonitoringOperator | `operatorId`, `name` | SOS 접수와 구조 완료를 수행하는 관제자 표현 |
...
현재 MVP는 데이터 규모가 작아 메모리 기반 구조를 사용한다. 운영 환경에서는 사용자와 위치 기록을 관계형 데이터베이스 또는 시계열 데이터베이스에 저장하고, 위험 지역 검색에는 공간 인덱스를 적용해야 한다.
...
현재 시스템은 상태 값을 저장하여 처리 단계를 구분한다. 향후 이전 상태별 허용 전이를 서버에서 검증하면 중복 접수와 완료된 SOS의 재처리를 더 엄격하게 방지할 수 있다.
...
- 운영 환경에서는 서버 재시작 후 데이터를 복구할 수 있어야 한다.
...
현재 MVP는 위치 공유 종료 기능만 제공하며 운영자 인증, HTTPS, 감사 로그와 영구 보존 정책은 구현하지 않았다.
...
|---|---|---|---|---|---|
| R-01 | 사용자 접속 | UC-01 | Client Registry | `POST /api/clients` | 완료 |
...
| R-07 | 구조 지원 | UC-07 | SOS State | SOS update API | 기본 상태 테스트 완료 |
| R-08 | 경로 분석 | UC-08 | RoutePoint | route API | 일부 완료 |
| R-09 | 시스템 종료 | UC-09 | Client connectionStatus | disconnect API | 테스트 없음 |
| R-10 | 개인정보 보호 | 공통 | Access Policy, Audit Log | 운영 기능 미구현 | 미완료 |
