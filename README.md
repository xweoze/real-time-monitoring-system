...
재난 상황에서 취약계층의 위치와 상태를 실시간으로 확인하고, 위험 지역 진입과 SOS 요청에 대응하는 웹 기반 MVP입니다.
...
사용자 앱에서 이름을 입력해 연결한 후 위치를 전송하면 관제 화면에 즉시 표시됩니다. `위험 이동 체험` 버튼을 누르면 위험 지역 진입 경고를 확인할 수 있으며, SOS 버튼을 3초간 누르면 관제 대시보드에 긴급 요청이 생성됩니다.
...
- 사용자 앱 위험 경고
- 3초 길게 누르는 SOS 요청
- 관제 화면 실시간 사용자, 위험, SOS 현황
- SOS 구조 접수 및 완료 처리
- 사용자별 이동 경로 표시
- SSE 기반 실시간 화면 갱신
- 인메모리 위치 이력과 이벤트 타임라인
...
> 현재 버전은 시연용 MVP로, 서버를 재시작하면 데이터가 초기화됩니다. 실제 배포 시에는 HTTPS, 운영자 인증, 데이터베이스와 개인정보 보존 정책을 추가해야 합니다.
