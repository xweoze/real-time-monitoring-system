...
| 2026-06-14 | 1.30 | 안전 분석·Webhook·재난문서 검색·Playwright 검증 반영 | 김지훈 |
...
- SOS 요청을 접수·담당자 배정·출동·완료 상태로 관리한다.
...
| External Automation | 선택형 HMAC 서명 n8n Webhook과 비동기 전송 큐 |
| Disaster Knowledge | 로컬 Markdown 행동요령의 출처 기반 검색 |
...
- 사용자 최신 위치와 위험 지역 표시
- 원형·다각형 지오펜스와 진입·이탈 기록
- 장시간 무동작·신호 끊김·배터리 부족 기기 경고
- 이동·정차·방문 장소 통계와 가까운 안전시설 안내
- 사용자 그룹 지도 필터, 알림 다중 조건 검색과 관리자 감사 로그
- HMAC 서명 기반 n8n Webhook과 비동기 알림 큐
- 공식 재난문서의 키워드·역문서빈도 기반 검색과 출처 표시
- Playwright 데스크톱·모바일 시각 회귀검사
- 관제자 요청 기반 역지오코딩 주소 확인
- 사용자 검색과 상세 정보 제공
- 최근 이동 경로 조회
- SOS `OPEN → ACKNOWLEDGED → DISPATCHED → RESOLVED` 처리
- 단계별 담당자, 처리 메모와 변경 시각 표시
...
classDiagram
    ...
    MonitoringStore --> WebhookNotifier
    Handler --> DisasterKnowledgeBase
    Handler --> ReverseGeocoder
...
| `update_sos()` | 관제자의 SOS 접수·배정·출동·완료·취소 및 처리 이력 저장 |
...
| `client_analytics()` | 최근 이동 경로의 거리·정차·방문 빈도 계산 |
| `nearby_facilities()` | 현재 위치와 안전시설의 거리를 계산해 가까운 순으로 반환 |
| `WebhookNotifier.enqueue()` | 중요 이벤트를 제한된 비동기 큐에 추가 |
| `DisasterKnowledgeBase.answer()` | 관련 문단과 공식 출처를 함께 반환 |
| `ReverseGeocoder.reverse()` | 요청 시 최신 좌표를 주소로 변환하고 캐시 |
...
stateDiagram-v2
    [*] --> OPEN: SOS 생성
    OPEN --> ACKNOWLEDGED: 관제 접수와 담당자 배정
    ACKNOWLEDGED --> DISPATCHED: 현장 출동
    DISPATCHED --> RESOLVED: 구조 완료
    OPEN --> CANCELLED: 요청 취소
    ACKNOWLEDGED --> CANCELLED: 상황 종료
    DISPATCHED --> CANCELLED: 출동 취소
```

서버는 현재 상태에 따른 허용 전이를 검증하여 `OPEN` 상태에서 바로 `RESOLVED`로 건너뛰는 요청 등을 거부한다.
각 상태 변경에는 담당 관리자, 선택형 처리 메모와 변경 시각을 기록한다.
...
| `GET` | `/api/safety-guides` | 재난 종류별 기본 행동요령 | `200` |
| `GET` | `/api/disaster-knowledge` | 재난문서 목록과 검색 범위 | `200` |
...
| `GET` | `/api/guardians` | 사용자가 연결한 보호자 목록 | `200` |
| `GET` | `/api/guardian/wards` | 보호자의 제한된 보호 대상 상태 | `200` |
| `GET` | `/api/clients/{id}/route` | 사용자 이동 경로 조회 | `200` |
| `GET` | `/api/clients/{id}/analytics` | 이동 거리·정차·방문 통계 | `200` |
| `GET` | `/api/clients/{id}/facilities` | 가까운 안전시설 조회 | `200` |
| `POST` | `/api/auth/signup` | 일반 사용자 회원가입과 세션 발급 | `201` |
| `POST` | `/api/auth/login` | 로그인과 12시간 세션 발급 | `200` |
| `POST` | `/api/auth/logout` | 현재 서버 세션 폐기 | `200` |
| `POST` | `/api/disaster-knowledge/search` | 출처 기반 재난문서 검색 | `200` |
| `POST` | `/api/operators` | 지역 관리자 계정 생성 | `201` |
| `POST` | `/api/danger-areas` | 원형·다각형 위험구역 생성 | `201` |
| `DELETE` | `/api/danger-areas/{id}` | 위험구역 삭제 | `200` |
| `POST` | `/api/groups` | 사용자 그룹 생성 | `201` |
| `POST` | `/api/groups/{id}/members` | 그룹 구성원 배정 | `200` |
| `DELETE` | `/api/groups/{id}` | 사용자 그룹 삭제 | `200` |
| `POST` | `/api/guardians/link` | 사용자가 보호자 계정 연결 | `201` |
| `DELETE` | `/api/guardians/{id}` | 보호자 연결 해제 | `200` |
...
| `POST` | `/api/clients/{id}/address` | 최신 좌표의 주소 확인 | `200` |
...
- 설정된 Kakao 또는 Nominatim 공급자로 최신 좌표의 주소를 요청 시 변환한다.
- 좌표를 소수점 넷째 자리 단위로 캐시하고 위치 변경 중 도착한 과거 응답은 저장하지 않는다.
...
- `USER`, `GUARDIAN`, `REGIONAL_OPERATOR`, `NATIONAL_ADMIN` 역할을 서버에서 검사한다.
- 보호자는 사용자가 직접 연결한 대상의 최신 안전 상태와 위치만 조회할 수 있다.
- 보호자는 위치 전송, SOS 생성, 전체 이동 경로와 관제 API에 접근할 수 없다.
...
현재 Python 자동 테스트 52개는 환경설정 로딩, 인증·세션, 보호자 연결, 원형·다각형 위험 판정,
기기 경고, 이동 통계, 그룹과 감사 로그, 프로필 검증,
Webhook 선별·서명, 재난문서 검색·출처, 회원 조회·수정·삭제,
세션 연쇄 삭제, 고아 단말 정리, 위치·주소·SOS,
지역 필터, 상태 복구와 공공데이터 변환을 검증한다.
Playwright는 데스크톱 로그인, 데스크톱 관제 레이아웃과 모바일 관제 화면의
가로 넘침·지도·패널 배치를 기준 이미지 3개로 검증한다. 동시접속 수와 지연 시간은
아직 부하 테스트로 검증하지 않았다.
...
| Test | `unittest` 52개 + Playwright 화면 회귀 3개 | API, 보안, 장애와 부하 테스트 확대 |
...
| R-04 | 사용자 위험 경고 | UC-04 | location 응답, client UI | 도메인 테스트 완료 |
| R-05 | SOS 생성 | UC-05 | SOS API, `create_sos()` | SOS 테스트 |
| R-06 | 사용자 관제 | UC-06 | state API, SSE, dashboard | Playwright 화면 검사 완료 |
...
| R-16 | 위험구역·기기 경고 | UC-14 | danger area, heartbeat | 자동 테스트 완료 |
| R-17 | 이동 분석·안전시설 | UC-15 | analytics, facilities API | 자동 테스트 완료 |
| R-18 | 그룹·감사 관리 | UC-16 | group API, audit snapshot | 자동 테스트 완료 |
| R-19 | 보호자 조회 | UC-17 | guardian API, role policy | 자동 테스트 완료 |
| R-20 | 외부 알림 자동화 | UC-18 | `WebhookNotifier` | 서명·선별 테스트 완료 |
| R-21 | 재난문서 검색 | UC-19 | `DisasterKnowledgeBase` | 검색·출처 테스트 완료 |
...
| Danger Area | 중심·반경의 원형 또는 꼭짓점 목록의 다각형 위험 지역 |
