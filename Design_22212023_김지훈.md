...
| Data Storage | 서버 메모리 내 해시맵과 제한 리스트 |
...
브라우저나 네트워크가 갑자기 종료된 경우를 감지하는 별도 heartbeat는 현재 구현에 없다.
...
현재 서버는 상태 문자열의 유효성만 검사한다. 운영 버전에서는 현재 상태에 따른 허용 전이도 검증해야 한다.
...
운영 버전에서는 `deque`, 데이터베이스, 공간 인덱스와 데이터 보존 정책을 사용한다.
...
| 서버 재시작 | 사용자 앱 세션 초기화 | 데이터베이스와 재인증 |
...
- 비정상 종료 사용자를 자동으로 `OFFLINE` 처리하는 heartbeat가 없다.
...
| Storage | Memory | PostgreSQL/PostGIS 또는 적합한 영구 저장소 |
